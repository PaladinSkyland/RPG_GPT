...
class Player:
    ...
    def get_max_hp(self):
        return self.base_hp + 10 * (self.get_level() - 1) + sum([item.hp for item in self.items.values()])
    ...
    def get_defense(self):
        return self.base_defense + (self.get_level() - 1) + sum([item.defense for item in self.items.values()]) + sum([status.defense for status in self.statuseffect.values()])
