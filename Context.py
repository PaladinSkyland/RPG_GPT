...
class Context:
    ...
    def set_context_from_json(self, json):
        ...
        for index, action in enumerate(json['for_player']['Possible Actions']):
            self.add_action(action, next(iter(json['for_game_master']['Possible Actions Description'][index:index+1]), "No description"))

    def add_action(self, action, description):
        self.actions.append({'title': action["title"], 'description': description, 'icon': action["icon"]})
